...
class OllamaClient:
    ...
    # Auto-coupled so EMBED_DIMENSIONS in .env is optional
    _PROVIDER_DIMS = {
        "ollama": 768,   # nomic-embed-text default
        "google": 768,   # gemini-embedding-001 with outputDimensionality=768
        "jina":   1024,  # jina-embeddings-v3 default
        "openai": 1536,  # text-embedding-3-small default
        "cohere": 1024,  # embed-english-v3 default
    }

    @classmethod
    def dims_for_provider(cls, provider: str) -> int:
        """Return the correct embedding dimensions for a given provider."""
    ...
    async def _embed_jina(self, texts: list[str]) -> list[list[float]]:
        ...
        async with httpx.AsyncClient(timeout=120) as c:
            for i in range(0, len(texts), BATCH):
                ...
                clean_batch = [t if t else "." for t in clean_batch]

                # Jina v3 max: 8192 tokens ≈ 32000 chars — truncate long texts
                MAX_CHARS = 30000
                clean_batch = [t[:MAX_CHARS] if len(t) > MAX_CHARS else t
                ...
                payload = {
                    ...
                    "truncate": True,  # let Jina truncate if still over limit
                }
                for attempt in range(5):
                    try:
                        ...
                        if r.status_code == 400:
                            ...
                            _log.error("Jina 400 batch %d — body: %s — first text: %s",
                                       i // BATCH, r.text[:200],
                                       repr(clean_batch[0][:80]) if clean_batch else "empty")
                            # Try splitting batch in half to isolate bad chunk
                            if len(clean_batch) > 1 and attempt == 0:
                                mid = len(clean_batch) // 2
                                for half in [clean_batch[:mid], clean_batch[mid:]]:
                                    if not half:
                                        continue
                                    hp = {**payload, "input": half}
                                    hr = await c.post(url, json=hp, headers=headers)
                                    if hr.status_code == 200:
                                        for item in sorted(hr.json()["data"], key=lambda x: x["index"]):
                        ...
                        if attempt == 4:
                            _log.error("Jina batch %d failed after 5 attempts: %s", i//BATCH, _e)
